      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end overflow-hidden selection:bg-[#ff5a00] selection:text-white">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-[#121620]/98 backdrop-blur-2xl text-white h-full flex flex-col shadow-2xl border-l border-white/15 z-10"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 bg-[#0d1017]">
              <div className="flex items-center gap-3">
                <div className="relative p-2 bg-[#ff5a00]/15 rounded-xl border border-[#ff5a00]/30 text-[#ff5a00]">
                ...
                <div>
                  <h2 className="text-base font-extrabold text-white tracking-tight flex items-center gap-2">
                    ...
                    {totalItems > 0 && (
                      <span className="bg-[#ff5a00] text-white text-xs font-bold px-2 py-0.5 rounded-full">
                        {totalItems}
                      </span>
                    )}
                  </h2>
                  <p className="text-xs text-gray-400">Resumen de tu pedido Balkran</p>
                </div>
              </div>
            ...
            {items.length === 0 ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-5">
                <div className="w-20 h-20 rounded-full bg-[#181d29] border border-white/10 flex items-center justify-center text-gray-500 shadow-inner">
                ...
                <Link
                  href="/productos"
                  onClick={closeCart}
                  className="mt-2 inline-flex items-center gap-2 bg-[#ff5a00] hover:bg-[#e04f00] text-white text-xs font-bold uppercase tracking-wider py-3 px-6 rounded-xl shadow-lg shadow-[#ff5a00]/30 transition-all active:scale-95"
                >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                ...
                <div className="space-y-3.5">
                  {items.map((item) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      className="flex items-center gap-3.5 p-3.5 bg-[#090b10]/90 border border-white/10 rounded-2xl hover:border-white/20 transition-all group"
                    >
                      {/* Product Thumbnail */}
                      <div className="relative w-16 h-16 rounded-xl bg-white/5 p-1 shrink-0 overflow-hidden border border-white/10">
                        <Image
                          src={item.imagen}
                          alt={item.nombre}
                          fill
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-start justify-between gap-2">
                          <h4 className="text-xs sm:text-sm font-bold text-white truncate group-hover:text-[#ff5a00] transition-colors">
                            {item.nombre}
                          </h4>
                          <button
                        ...
                        {item.linea && (
                          <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-[#ff5a00] bg-[#ff5a00]/10 px-2 py-0.5 rounded-md">
                            {item.linea}
                          </span>
                        )}
                ...
                <form id="whatsapp-checkout-form" onSubmit={handleCheckoutWhatsApp} className="space-y-3 pt-4 border-t border-white/10">
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      className="w-full bg-[#090b10]/90 border border-white/15 focus:border-[#ff5a00] rounded-xl text-white text-xs px-3.5 py-2.5 placeholder:text-gray-500 focus:outline-none transition-all"
                    />
                ...
                {/* Guarantee Banner */}
                <div className="p-3 bg-[#0d1017] border border-white/10 rounded-xl flex items-center gap-3 text-xs text-gray-400">
            ...
            {items.length > 0 && (
              <div className="p-6 border-t border-white/10 bg-[#0d1017] space-y-4">
                {/* Total Calculations */}
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between text-gray-400">
                    <span>Subtotal de productos:</span>
                    <span className="font-semibold text-white">{formatCOP(totalPrice)}</span>
                  </div>
                  <div className="flex justify-between text-gray-400">
                    ...
                    <span className="text-emerald-400 font-semibold">Por coordinar</span>
                  </div>
                  <div className="border-t border-white/10 pt-2 flex justify-between items-baseline">
                    <span className="text-sm font-bold text-white uppercase tracking-wider">TOTAL DE COMPRA:</span>
                ...
                <button
                  ...
                  className="w-full bg-[#25D366] hover:bg-[#20bd5a] active:scale-[0.99] text-white font-display text-xs sm:text-sm font-bold uppercase tracking-wider py-4 rounded-xl shadow-lg shadow-[#25D366]/30 hover:shadow-[#25D366]/50 transition-all flex items-center justify-center gap-2.5"
                >
