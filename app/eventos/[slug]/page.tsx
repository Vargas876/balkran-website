...
import { pick, pickOpt } from '@/lib/i18n';
...
const L: Record<'es' | 'en' | 'fr', Record<string, string>> = {
  es: {
    ...
    'entity': 'Entidad:',
    'evaluatedStandard': 'Estándar Evaluado:',
    'officialDoc': 'Documento Oficial',
    'fullPdf': 'Informe Completo en PDF',
    'accessDoc': 'Accede al documento digital oficial con el detalle de los indicadores y fotografías del proyecto.',
    'downloadDoc': 'Descargar Documento',
    'exploreMore': '¿Quieres explorar más informes?',
    'checkHistory': 'Revisa el historial completo de eventos, capacitaciones y reportes BIC de Balkran.',
    'viewAllEvents': 'Ver Todos los Eventos',
    'executiveSummary': 'Resumen Ejecutivo',
    'section': 'Sección',
    'openImage': 'Haz clic para abrir la imagen en alta resolución',
    'viewFullSize': 'Ver tamaño completo',
    'relatedPosts': 'Publicaciones Relacionadas',
    'otherReports': 'Otros Informes y Eventos Destacados',
  ...
  en: {
    ...
    'entity': 'Entity:',
    'evaluatedStandard': 'Standard Assessed:',
    'officialDoc': 'Official Document',
    'fullPdf': 'Complete Report in PDF',
    'accessDoc': 'Access the official digital document with the details of the indicators and project photographs.',
    'downloadDoc': 'Download Document',
    'exploreMore': 'Want to explore more reports?',
    'checkHistory': 'Browse the complete history of events, trainings and BIC reports from Balkran.',
    'viewAllEvents': 'View All Events',
    'executiveSummary': 'Executive Summary',
    'section': 'Section',
    'openImage': 'Click to open the image in high resolution',
    'viewFullSize': 'View full size',
    'relatedPosts': 'Related Publications',
    'otherReports': 'Other Featured Reports and Events',
  ...
  fr: {
    ...
    'entity': 'Entité :',
    'evaluatedStandard': 'Norme évaluée :',
    'officialDoc': 'Document officiel',
    'fullPdf': 'Rapport complet en PDF',
    'accessDoc': 'Accédez au document numérique officiel avec le détail des indicateurs et des photographies du projet.',
    'downloadDoc': 'Télécharger le document',
    'exploreMore': 'Souhaitez-vous explorer d\'autres rapports ?',
    'checkHistory': 'Consultez l\'historique complet des événements, formations et rapports BIC de Balkran.',
    'viewAllEvents': 'Voir tous les événements',
    'executiveSummary': 'Résumé exécutif',
    'section': 'Section',
    'openImage': 'Cliquez pour ouvrir l\'image en haute résolution',
    'viewFullSize': 'Voir en taille réelle',
    'relatedPosts': 'Publications connexes',
    'otherReports': 'Autres rapports et événements en vedette',
    ...
    <main className="min-h-screen bg-[#f8fafc] text-[#1a2130] font-sans">
      ...
      <section className="relative bg-white text-[#111111] border-b border-gray-200/80 overflow-hidden pt-32 sm:pt-36 lg:pt-40 pb-12 sm:pb-16">
        ...
        <div className="relative max-w-[1500px] mx-auto px-6 sm:px-10 lg:px-16">
          ...
          >
            <ArrowLeft className="w-3.5 h-3.5" /> {l('backToEvents')}
          </Link>
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="bg-slate-100 border border-slate-200 text-[#1a2130] text-xs font-bold uppercase tracking-wide px-3.5 py-1.5 rounded-full inline-flex items-center gap-1.5">
                  ...
                  {formatFecha(evento.fechaISO, lang)}
                </span>
                {evento.esInforme && (
            ...
            <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-28">
              ...
              <div className="bg-white rounded-3xl border border-gray-200/90 shadow-sm p-6 sm:p-8 space-y-5">
                <div className="flex items-center gap-3 border-b border-gray-100 pb-4">
                  ...
                  <div>
                    <h3 className="font-display font-extrabold text-lg text-[#111111]">{l('eventSheet')}</h3>
                    <p className="text-xs text-[#565e6e]">{l('recordInfo')}</p>
                  </div>
                </div>

                <div className="space-y-3.5 text-xs text-[#565e6e]">
                  <div className="flex items-center justify-between py-1 border-b border-gray-100">
                    <span className="font-bold text-[#1a2130]">{l('category')}</span>
                    <span className="font-bold text-[#ff5a00] uppercase">{pick(lang, evento.categoria)}</span>
                  </div>
                  <div className="flex items-center justify-between py-1 border-b border-gray-100">
                    <span className="font-bold text-[#1a2130]">{l('publishDate')}</span>
                    <span>{formatFecha(evento.fechaISO, lang)}</span>
                  </div>
                  <div className="flex items-center justify-between py-1 border-b border-gray-100">
                    <span className="font-bold text-[#1a2130]">{l('entity')}</span>
                    <span>BALKRAN INC S.A.S BIC</span>
                  </div>
                  {evento.esInforme && (
                    <div className="flex items-center justify-between py-1 border-b border-gray-100">
                      <span className="font-bold text-[#1a2130]">{l('evaluatedStandard')}</span>
              ...
              <div className="bg-slate-100 border border-slate-200 rounded-3xl p-6 space-y-3">
                <h4 className="font-display font-bold text-sm text-[#111111]">{l('exploreMore')}</h4>
                ...
                <div key={si} className="bg-white rounded-3xl border border-gray-200/90 shadow-sm p-8 space-y-6">
                  {sec.titulo && (
                    <div className="border-b border-gray-100 pb-3">
                      <span className="text-[#ff5a00] font-display text-[11px] font-extrabold uppercase tracking-widest block">{l('section')} {si + 1}</span>
                      <h2 className="font-display font-extrabold text-2xl text-[#111111] tracking-tight">
                        {pick(lang, sec.titulo)}
                      </h2>
                    </div>
                  )}
                    ...
                    <div className="space-y-3 pt-2">
                      {sec.lista.map((item, li) => (
                        <div key={li} className="flex items-start gap-3 bg-slate-50 rounded-2xl p-4 border border-slate-100">
                          ...
                          <p className="text-sm text-[#1a2130] font-medium leading-relaxed font-sans">{pick(lang, item)}</p>
                        </div>
                      ))}
                    </div>
                  )}
                      ...
                      {sec.imagenes.map((img, ii) => (
                        <figure key={ii} className="space-y-2 group">
                          ...
                          >
                            <Image
                              src={img.src}
                              alt={pickOpt(lang, img.caption) || pick(lang, evento.titulo)}
                              fill
                            ...
                            <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <span className="bg-white/90 backdrop-blur-md text-[#111111] text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow">
                                <ZoomIn className="w-4 h-4 text-[#ff5a00]" /> {l('viewFullSize')}
                              </span>
                            </div>
                          </a>
                          {img.caption && (
            ...
            {otrosEventos.map((item) => (
              ...
              >
                <div>
                  ...
                  <div className="p-6 space-y-3">
                    <span className="text-xs text-gray-400 font-medium block">
                      {formatFecha(item.fechaISO, lang)}
                    </span>
                    ...
                    <p className="text-xs text-[#565e6e] leading-relaxed line-clamp-3">
                      {pick(lang, item.descripcionCorta) || pick(lang, item.resumen)}
                    </p>
                  </div>
                </div>
                ...
                <div className="p-6 pt-0">
                  ...
                  >
                    <span>{item.esInforme ? l('viewReport') : l('viewDetails')}</span>
